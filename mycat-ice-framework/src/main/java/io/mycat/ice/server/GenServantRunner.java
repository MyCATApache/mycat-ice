...
public class GenServantRunner {
	...
	public static void main(String[] args) {
		if (args.length < 2) {
			System.out.println(
					"Missing params: serviceName servantClassName \r\n Example:\r\n  MyService com.my.demo.MyServiceImpl");
			System.exit(-1);
		}
		...
		try {
			...
			ObjectAdapter adapter = ic.createObjectAdapterWithEndpoints(serviceName + "Adapter", "default -p 20000");
			// 实例化一个服务对象(Servant)
			Object servant = (Object) Class.forName(servantClass).newInstance();
			// 将Servant增加到ObjectAdapter中，并将Servant关联到ID为MyService的Ice Object
			adapter.add(servant, Util.stringToIdentity(serviceName));
			// 激活ObjectAdapter
			adapter.activate();
			// 让服务在退出之前，一直持续对请求的监听
